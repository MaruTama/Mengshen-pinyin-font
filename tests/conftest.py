...
def performance_thresholds():
    """Performance thresholds for benchmark testing."""
    return {
        "font_generation_time": 300,  # seconds
        "memory_usage_mb": 2048,  # MB
        "test_execution_time": 300,  # seconds
        "glyph_processing_rate": 100,  # glyphs per second
    }


# Security testing fixtures
@pytest.fixture
def security_test_environment(temp_dir):
    """Create isolated environment for security testing."""
    # Create fake sensitive files to test access prevention
    sensitive_files = {
        "passwd": temp_dir / "etc" / "passwd",
        "shadow": temp_dir / "etc" / "shadow",
        "config": temp_dir / "config" / "secret.conf",
    }

    for file_path in sensitive_files.values():
        file_path.parent.mkdir(parents=True, exist_ok=True)
        file_path.write_text("SENSITIVE_DATA_DO_NOT_ACCESS")
...
# Characterization testing fixtures
@pytest.fixture
def legacy_output_baseline():
    """Provide baseline data for legacy compatibility testing."""
    return {
        "expected_glyph_count": {
            "han_serif": 63140,
            "handwritten": 63140,
        },
        "expected_font_size_range": {
            "han_serif": (16000000, 18000000),  # bytes
            "handwritten": (16000000, 18000000),  # bytes
        },
        "expected_tables": [
            "cmap",
            "cmap_uvs",
