...
def get_pinyin_table_with_mapping_table():
    """
    Load pinyin mapping table from merged-mapping-table.txt.
    
    This file is generated by make_unicode_pinyin_map_table.py and already
    contains processed data with overwrite.txt applied, so we just read it directly.
    """
    pinyin_table = {}
    merged_file = os.path.join(p.DIR_OUTPUT, MERGED_MAPPING_TABLE)
    
    with open(merged_file, encoding='utf-8') as read_file:
