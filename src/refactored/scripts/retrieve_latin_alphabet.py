...
from refactored.utils.dict_utils import deepupdate
...
class LatinAlphabetRetriever:
    ...
    def get_cmap_table(self, source_font_json: str) -> Dict[str, str]:
        ...
        try:
            output = self.shell.execute(cmd, capture_output=True)
    ...
    def filter_alphabet_glyphs(self, cmap_table: Dict[str, str], glyf_table: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def save_alphabet_json(self, alphabet_glyphs: Dict[str, Any], output_path: str) -> None:
    ...
    def retrieve_alphabet(self, source_font_name: str, style: str) -> None:
        ...
        try:
            ...
            print(f"Successfully extracted {len(alphabet_glyphs)} alphabet glyphs for {style} style")
...
def retrieve_alphabet_main(args: Optional[List[str]] = None) -> None:
    ...
    if font_type == FontType.HAN_SERIF:
        ...
        source_font = "res/fonts/handwritten/latin-alphabet-of-SetoFont-SP.ttf"
