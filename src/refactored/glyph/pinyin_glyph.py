...
from refactored.utils.hanzi_pinyin import HanziPinyin
...
class PinyinGlyph:
    ...
    def __init__(self, template_main_json: str, alphabet_for_pinyin_json: str, font_type: FontType):
        """
    ...
    def _create_pinyin_glyph(self, pronunciation: str) -> Dict[str, Any]:
        ...
        # 重なりを避けるモードの適用
        if (self.METADATA_FOR_PINYIN.is_avoid_overlapping_mode and 
            len(pronunciation) >= 5):
            scale_x *= (1.0 - self.METADATA_FOR_PINYIN.x_scale_reduction_for_avoid_overlapping)
