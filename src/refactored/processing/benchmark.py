...
class Benchmark:
        ...
        for iteration in range(iterations):
            if self.memory_profiler:
                with self.memory_profiler.monitor_memory(
                    f"{func_name}_iteration_{iteration}"
                ):
    ...
    def compare_functions(
        self, functions: Dict[str, Callable], *args, iterations: int = 10, **kwargs
    ) -> BenchmarkSuite:
        """
    ...
    def profile_with_comparison(
        ...
        **kwargs,
        ...
        """
        old_result = self.time_function(
            old_func, *args, iterations=iterations, **kwargs
        )
        old_result.name = "Old Implementation"

        new_result = self.time_function(
            new_func, *args, iterations=iterations, **kwargs
        )
        new_result.name = "New Implementation"

        # Calculate improvements
        time_improvement = (
            (old_result.mean_time - new_result.mean_time) / old_result.mean_time * 100
        )
        memory_improvement = None

        if old_result.memory_usage and new_result.memory_usage:
            memory_improvement = (
                (old_result.memory_usage - new_result.memory_usage)
                / old_result.memory_usage
                * 100
            )
        ...
        return {
            ...
            "improvements": {
                ...
                "speedup_factor": (
                    old_result.mean_time / new_result.mean_time
                    if new_result.mean_time > 0
                    else float("inf")
                ),
            },
        }
...
class FontGenerationBenchmark:
    ...
    def benchmark_character_processing(self, characters: List[str]) -> BenchmarkResult:
        ...
        def process_characters():
            ...
            for char in characters:
                ...
                if char_info:
                    simplified = utility.simplify_pronunciation(
                        char_info.pronunciations[0]
                    )
        ...
        return self.benchmark.time_function(
            process_characters, iterations=5, warmup_iterations=1
        )

    def benchmark_parallel_vs_serial(
        self, data: List[Any], processing_func: Callable
    ) -> Dict[str, Any]:
        ...
        return self.benchmark.profile_with_comparison(
            serial_processing, parallel_processing, iterations=5
        )

    def benchmark_caching_impact(
        self, operation_func: Callable, test_data: List[Any]
    ) -> Dict[str, Any]:
        ...
        cache_improvement = (
            (cold_result.mean_time - warm_result.mean_time)
            / cold_result.mean_time
            * 100
        )
    ...
    def run_comprehensive_benchmark(
        self, sample_characters: List[str]
    ) -> Dict[str, Any]:
        ...
        with self.benchmark.benchmark_context("Caching Impact"):
            ...
            cache_result = self.benchmark_caching_impact(
                utility.simplify_pronunciation, ["zhōng", "zhòng", "píng", "yīn"] * 25
            )
...
def run_performance_comparison() -> None:
    ...
    # sample_pronunciations = ["zhōng", "guó", "pīn", "yīn", "zì", "tǐ"] * 20  # Unused
    ...
    for test_name, result in results.items():
        ...
        if isinstance(result, BenchmarkResult):
            ...
            print(
                f"  Time improvement: {result['improvements']['time_improvement_percent']:.1f}%"
            )
            print(f"  Speedup factor: {result['improvements']['speedup_factor']:.2f}x")
            if result["improvements"]["memory_improvement_percent"]:
                print(
                    f"  Memory improvement: {result['improvements']['memory_improvement_percent']:.1f}%"
                )
