...
class GSUBTable:
    ...
    def load_pattern_table(self):
        ...
        with open(self.PATTERN_ONE_TXT, mode="r", encoding="utf-8") as read_file:
            for line in read_file:
                ...
                tmp.update(
                    {hanzi: {"variational_pronunciation": pinyin, "patterns": patterns}}
                )
    ...
    def make_rclt0_feature(self):
        ...
        for idx in range(max_num_of_variational_pinyin):
            ...
            lookup_tables.update(
                {lookup_name: {"type": "gsub_single", "flags": {}, "subtables": [{}]}}
            )
        ...
        for idx in range(max_num_of_variational_pinyin):
            ...
            for apply_hanzi in self.pattern_one[idx].keys():
                ...
                lookup_table_subtables.update(
                    {
                        apply_hanzi_cid: "{}.ss{:02}".format(
                            apply_hanzi_cid, pg.SS_VARIATIONAL_PRONUNCIATION + idx
                        )
                    }
                )
                # to rclt0
                ...
                other_match = [
                    s for s in patterns if not (s in (left_match + right_match))
                ]

                if len(left_match) > 0:
                    context_hanzi_cids = [
                        utility.convert_str_hanzi_2_cid(context_hanzi)
                        for context_hanzi in [
                            context_hanzi.replace("~", "")
                            for context_hanzi in left_match
                        ]
                    ]
                ...
                if len(right_match) > 0:
                    context_hanzi_cids = [
                        ...
                        for context_hanzi in [
                            context_hanzi.replace("~", "")
                            for context_hanzi in right_match
                        ]
                    ]
                    list_rclt_0_subtables.append(
                        {
                            ...
                            "apply": [
                                {"at": 1, "lookup": "lookup_pattern_0{}".format(idx)}
                            ],
                            "inputBegins": 1,
                            "inputEnds": 2,
                        }
                ...
                for match_pattern in other_match:
                    ...
                    list_rclt_0_subtables.append(
                        {
                            "match": [
                                [utility.convert_str_hanzi_2_cid(hanzi)]
                                for hanzi in match_pattern.replace("~", apply_hanzi)
                            ],
                            "apply": [
                                {"at": at, "lookup": "lookup_pattern_0{}".format(idx)}
                            ],
                            "inputBegins": at,
                            "inputEnds": at + 1,
                        }
    ...
    def make_rclt1_feature(self):
        ...
        for lookup_name, table in self.pattern_two["lookup_table"].items():
            ...
            lookup_table_subtables.update(
                {
                    utility.convert_str_hanzi_2_cid(k): v.replace(
                        k, utility.convert_str_hanzi_2_cid(k)
                    )
        ...
        for phrase, list_pattern_table in self.pattern_two["patterns"].items():
            ...
            list_rclt_1_subtables.append(
                {
                    "match": [
                        [utility.convert_str_hanzi_2_cid(hanzi)] for hanzi in phrase
                    ],
                    "apply": applies,
                    "inputBegins": min(ats),
                    "inputEnds": max(ats) + 1,
        ...
        for phrase, setting_of_phrase in self.exception_pattern["patterns"].items():
            ...
            if ignore_pattern != None:
                ...
                if len(tmp) == 1:
                    ...
                    raise Exception(
                        "exception pattern の ignore 記述が間違っています。: \n {}".format(
                            ignore_pattern
                        )
                    )
                # 空白とシングルコートを削除
