...
# 単語中に同じ漢字が２回以上出現する単語を返す
# 兴兴头头 -> xīng/xìng/tou/tóu のように重なると音が変わるパターン
# 血淋淋 -> xiě/lín/lín のように音が同じパターンの２つがある
# これらの単語は別ファイルへ
def get_multiple_same_hanzi_in_phrase(PHRASE_TABLE_FILE):
    list_multiple_same_hanzi_in_phrase = []
    with open(PHRASE_TABLE_FILE) as read_file:
        for line in read_file:
            [phrase, _] = line.rstrip('\n').split(': ')
            multiple_charactor = [charactor for charactor in phrase if phrase.count(charactor) > 1]
            if len(multiple_charactor) > 0:
                list_multiple_same_hanzi_in_phrase.append(line.rstrip('\n'))
    return list_multiple_same_hanzi_in_phrase
...
def pattern_one(PHRASE_TABLE_FILE):    
    ...
    # """
    # list_multiple_same_hanzi_in_phrase = get_multiple_same_hanzi_in_phrase(PHRASE_TABLE_FILE)
    # if len(list_multiple_same_hanzi_in_phrase) > 0:
    #     print("単語を phrase_of_pattern_two.txt に移動させてください")
    #     print("Multiple occurrences of the same hanzi(kanji) in a phrase : ")
    #     print(list_multiple_same_hanzi_in_phrase)
    #     exit()
    # else:
    #     print("success!")
    #     print("There is only one hanzi in a phrase.")
    # print()
...
if __name__ == '__main__':
    ...
    # print( count_variational_pinyin("干吗", "gàn/má") )
