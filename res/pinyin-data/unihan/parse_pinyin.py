...
def re_match_pinyin_line(kind):
    return re.compile(r"^U\+(?P<code>[0-9A-Z]+)\t{}\t(?P<pinyin>.+)$".format(kind))


PINYIN = r"[^\d\.,]+"
re_khanyupinyin = re.compile(
    r"""
    (?:\d{5}\.\d{2}0,)*\d{5}\.\d{2}0:
    ((?:%(pinyin)s,)*)
    (%(pinyin)s)
