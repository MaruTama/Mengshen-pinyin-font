...
**実装完了状況**:
- [x] TypedDict から dataclass への移行 ✅ (src/refactored/config/)
- [x] Iterator と Generator によるメモリ効率化 ✅ (src/refactored/processing/)
...
├── utils/       # ユーティリティ ✅
│   ├── character_utils.py # 文字処理ユーティリティ
│   ├── cmap_utils.py     # cmap テーブルユーティリティ
│   ├── dict_utils.py     # 辞書ユーティリティ
│   ├── hanzi_pinyin.py   # 漢字拼音処理
│   ├── pinyin_conversion.py # 拼音変換
│   ├── pinyin_utils.py   # 拼音ユーティリティ
│   ├── shell_utils.py    # シェルユーティリティ
│   └── version_utils.py  # バージョンユーティリティ
└── cli/         # コマンドライン ✅
...
- 新実装: `python -m refactored.cli.main -t han_serif`
- 両方とも動作可能な状態を維持
...
# 新実装
time python -m refactored.cli.main -t han_serif
```
...
**実装完了状況:**
- [x] プロファイリング機能実装 ✅ (`src/refactored/processing/profiling.py`)
- [x] 高度なキャッシュシステム ✅ (`src/refactored/processing/cache_manager.py`) 
- [x] 並列処理エンジン ✅ (`src/refactored/processing/parallel_processor.py`)
- [x] 最適化されたユーティリティ ✅ (`src/refactored/processing/optimized_utility.py`)
- [x] ベンチマーク・比較ツール ✅ (`src/refactored/processing/benchmark.py`)
...
- `src/refactored/font/font_builder.py`は実装済みだが、旧パイプラインとの統合未完了
- 新CLI（`python -m refactored.cli.main`）は動作するが、バイナリレベルでの機能的同等性なし
...
# 新実装（動作するが出力異常）
python -m refactored.cli.main -t han_serif  # サイズ異常
...
**移植対象と進捗**:
- ✅ **utility.py** → `src/refactored/core/legacy_utility.py` (完了)
- ✅ **name_table.py** → `src/refactored/config/font_name_tables.py` (完了)
- ✅ **config.py** → `src/refactored/config/legacy_config.py` (完了)
...
**移植されたファイル**:
1. `src/refactored/core/legacy_utility.py` - ピンイン処理ユーティリティ
2. `src/refactored/config/font_name_tables.py` - フォント名テーブル定義
3. `src/refactored/config/legacy_config.py` - フォント設定とパス
...
- **対象ファイル**: `src/refactored/font/glyph_manager.py`

**2. 🔴 緊急: FontBuilder機能統合（バイナリ検証ベース）**
- **前提**: 拼音フォント修正完了後に実施
- **目標**: 新実装で旧実装とバイナリレベルで同等のフォント出力を実現
- **作業**: `src/refactored/font/font_builder.py`で移植されたモジュールを使用
- **検証手順**:
  ```bash
  # 1. 両実装でフォント生成
  python src/main.py -t han_serif
